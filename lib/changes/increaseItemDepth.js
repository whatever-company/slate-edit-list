...
function increaseItemDepth(opts, change, block) {
    const previousItem = getPreviousItem(opts, change.state, block);
    const currentItem = getCurrentItem(opts, change.state, block);

    if (previousItem) {
      // Move the item in the sublist of previous item
      return moveAsSubItem(opts, change, currentItem, previousItem.key);
    } else {
      const currentList = getListForItem(opts, change.state, currentItem);
